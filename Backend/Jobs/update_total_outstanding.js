...
const update_total_outstanding = async (loanID, interestComponent, emi_amount) => {
    console.log(loanID, interestComponent, emi_amount)
    try {
        ...
        if (!emiDetails.outstanding_added) {
            const [rows] = await connection.execute("UPDATE loan_details SET total_outstanding=total_outstanding + ? WHERE loan_id=?", [emi_amount + interestComponent, loanID]) 
